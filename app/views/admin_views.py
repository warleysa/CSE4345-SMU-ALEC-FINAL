...
def profilePage():
	...
	return render_template('tutor-profile.html', user=user, tutor_class_info=tutor_class_info, adminStatus=getAdminStatus())

@admin_blueprint.route('/session-check-in/<string:id>', methods=['GET'])
def checkInPage(id):
	currID = current_user.get_id()
	user = User.query.filter_by(id=currID).first()
	sqlClass=("SELECT tc.id as id, CONCAT(c.dept,' ',c.number) as class_info, c.class_name as class_name, u.full_name as tutor_name, u.location as location"
	...
	resultC = db.session.execute(sqlClass)
	tutor_class_info = resultC.fetchall()
	return render_template('tutor-session-check-in.html', user=user, tutor_class_info=tutor_class_info, adminStatus=getAdminStatus())

@admin_blueprint.route('/session-check-in', methods=['GET'])
def checkInPage():
	currID = current_user.get_id()
	user = User.query.filter_by(id=currID).first()
	sqlClass=("SELECT tc.id as id, CONCAT(c.dept,' ',c.number) as class_info, c.class_name as class_name, u.full_name as tutor_name, u.location as location"
...
@admin_blueprint.route('/alec-check-in', methods=['GET'])
def alecCheckInPage():
	currID = current_user.get_id()
	user = User.query.filter_by(id=currID).first()
	return render_template('tutor-alec-check-in.html', user=user)
...
def tutorAddClassPage(id):
	...
	if currID == str(id) or adminStatus == True: 
		...
		return render_template('tutor-profile-add-class.html', all_classes=all_classes, user_id=currID, adminStatus=getAdminStatus())
	else:
		flash(u'Invalid permissions to edit this profile', 'danger')
		return redirect('login')
...
@admin_blueprint.route('/class-check-in/<int:id>', methods=['POST'])
def tutorCheckInClass(id):
	currID = current_user.get_id()
	adminStatus = User.query.with_entities(User.admin_status).filter_by(id=currID).first().admin_status
	# Checks if user is admin OR is editing their own profile page
	if currID == str(id) or adminStatus == True: 
...
# @admin_blueprint.route('/tutor-alec-check-in/<int:id>', methods=['POST'])
# def tutorAddClass(id):
# 	currID = current_user.get_id()
# 	adminStatus = User.query.with_entities(User.admin_status).filter_by(id=currID).first().admin_status
# 	# Checks if user is admin OR is editing their own profile page
...
def statsPage(id):
	...
	if currID == str(id) or adminStatus == True: 
		...
		return render_template('stats.html', tutor_class_info=tutor_class_info, alec_sessions=alec_sessions, tutoring_sessions=tutoring_sessions, adminStatus=getAdminStatus())
	else:
...
def adminPage():
	...
	if currID == str(id) or adminStatus == True: 
		...
		return render_template('admin-page.html', classes=classes, tutors=tutors, admins=admins, adminStatus=getAdminStatus())
	else:
...
def update_class(id):
    ...
    return render_template('update_class.html', data=new_class, adminStatus=getAdminStatus())
