...
@api_blueprint.route('/alec-check-in/<int:id>', methods=['POST'])
def alec_check_in(id):
    ...
    if currID == str(id) or adminStatus == True: 
        sql=("INSERT INTO alec_session (user_id) VALUES ("+str(id)+")")
        db.session.execute(sql)
        db.session.commit()
        sql2 = ("SELECT id FROM alec_session WHERE id = LAST_INSERT_ID()")
        result = db.session.execute(sql2)
        session_result = result.fetchall()
        return redirect('session-check-in/'+str(session_result[0]))
    else:
        flash(u'There was a problem checking into the ALEC', 'danger')
        return None
