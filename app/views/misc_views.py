...
def member_page():
    sql=("SELECT c.id as id, CONCAT(c.dept,' ',c.number) as class_info, c.dept as dept, 1 as tutor_in_alec, (ts.start_time IS NULL) as availible"
        + " FROM classes c INNER JOIN tutor_classes tc ON c.id = tc.class_id"
        + " INNER JOIN alec_session als ON tc.user_id = als.user_id"
        + " LEFT JOIN tutor_session ts ON als.id = ts.alec_session_id"
        + " WHERE als.end_time IS NULL"
        + " GROUP BY class_info")
